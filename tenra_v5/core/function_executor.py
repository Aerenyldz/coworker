...
class TenraFunctionExecutor:
    """Tenra V6 — Tüm fonksiyonları çalıştıran merkezi motor."""
    ...
    def _get_path(self, params: Dict, default_to_desktop: bool = False) -> str:
        """Parametrelerden dosya/klasör yolunu çıkarır, normalize eder ve Linux-Windows uyumsuzluğunu düzeltir."""
        path = params.get("path", "")
        if not path:
            for alt in ("name", "filename", "file", "target", "folder", "directory", "dir"):
                if params.get(alt):
                    path = params.get(alt)
                    break
        path = str(path).strip()
        ...
        if not path:
            return self.desktop_path if default_to_desktop else ""

        # Linux absolute paths generated by LLM (like /home/user/Desktop/file or /Desktop/file) on Windows
        # will start with '/' or '\'. We need to make them relative to self.desktop_path.
        if path.startswith('/') or path.startswith('\\'):
            import re
            # Check if it has a Windows drive letter (like C:) or UNC network path prefix
            if not re.match(r'^[a-zA-Z]:', path) and not path.startswith('\\\\'):
                # Strip leading slashes and any home/Desktop prefixes
                clean_path = re.sub(r'^[/|\\\\]+(?:home[/|\\\\]+[^/|\\\\]+[/|\\\\]+Desktop|Desktop|home[/|\\\\]+user[/|\\\\]+Desktop)[/|\\\\]*', '', path, flags=re.IGNORECASE)
                path = clean_path
    ...
    def _click_text_on_screen(self, params: Dict) -> Dict:
        ...
        try:
            ...
            target_text = params.get("text", "").lower().strip()
            if not target_text:
            ...
            for i in range(n_data - n_target + 1):
                match = True
                for j in range(n_target):
                    word_in_data = str(data['text'][i + j]).lower().strip()
                    if not word_in_data or int(data['conf'][i + j]) < 40:
                        match = False
                        break
                    if target_words[j] not in word_in_data:
                        match = False
                        break
                ...
                if match:
                    x_start = data['left'][i]
                    x_end = data['left'][i + n_target - 1] + data['width'][i + n_target - 1]
                    y_start = data['top'][i]
                    h = data['height'][i]
    ...
    def _run_command(self, params: Dict) -> Dict:
        """PowerShell komutu çalıştır — stdout/stderr/exit_code ayrımıyla döndürür."""
        ...
        try:
            ...
            return {"success": False, "message": "Komut zaman aşımına uğradı (60sn)", "data": {"command": command, "stdout": "", "stderr": "Timeout", "exit_code": -1}}
        except Exception as e:
    ...
    def _create_file(self, params: Dict) -> Dict:
        ...
        try:
            ...
            return {"success": False, "message": f"Dosya oluşturma hatası: {e}", "data": None}
    ...
    def _delete_file(self, params: Dict) -> Dict:
        ...
        if os.path.exists(path):
            try:
                ...
                return {"success": False, "message": f"Silme hatası: {e}", "data": None}
    ...
    def _patch_file(self, params: Dict) -> Dict:
        ...
        try:
            ...
            # Line ending normalization to match the file's primary line ending style
            if "\r\n" in original and "\r\n" not in old_string:
    ...
    def _search_files(self, params: Dict) -> Dict:
        ...
        search_path = self._get_path(params, default_to_desktop=True)
    ...
    def _list_directory(self, params: Dict) -> Dict:
        ...
        path = self._get_path(params, default_to_desktop=True)
        if not os.path.isabs(path):
            path = os.path.join(self.desktop_path, path)
    ...
    def _rename_file(self, params: Dict) -> Dict:
        ...
        old_path = self._get_path(params) or params.get("old_path", "")
    ...
    def _web_search(self, params: Dict) -> Dict:
        ...
        try:
            try:
                ...
                from duckduckgo_search import DDGS  # fallback for older install
            with DDGS() as ddgs:
